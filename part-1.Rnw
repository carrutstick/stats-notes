...
\center{The simplest continuous distribution is the uniform distribution, which has a constant pdf.}
...
\frame{\frametitle{Combining Distributions}
\centering
Definition: \emph{Two random variables $x$ and $y$ are \textbf{independent} iff their joint pmf is the 
product of their individual pmfs $p(x,y) = p(x)p(y)$}\pause\vspace{12pt}
...
\begin{align*}
	p(g) \equiv \lim_{\epsilon\rightarrow0}P(G(X,Y) \in [g,g+\epsilon]) = \iint_{G(x,y)=g} p(x, y) dxdy
\end{align*}\pause
